...
public class Sorting {
    ...
    public static void sort(int[] array, Algorithm algo) {
        switch (algo) {
            ...
            case CYCLE_SORT ->
                cycleSort(array, 0, array.length-1);
        }
    }
...
// Three Static Helper Methods (one private and two public)
    ...
    private static void quickSort2(int[] array, int low, int high) {
        ...
        if (high > low) {
            pivotpoint = partition2(array, low, high);
    ...
    private static int partition2(int[] array, int low, int high) {
        ...
        else if(array[high]<array[middle] && array[low]<array[high]){
            median = high;
            swap(array, low, high);
        }
        else{
    ...
    private static void cycleSort(int[] array, int low, int high) {
        int writes = 0;
        
        for(int start = 0; start < high; start++){
            int val = array[start];
